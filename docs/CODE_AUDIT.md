...
| Trading | 模拟下单、Binance 余额/持仓/下单/K线/公告 | `submit_order` / `place_binance_order` / `cancel_binance_order` / `get_binance_*` | ✅ 已修复下单参数契约 |
...
| Settings | 全量配置读写、Binance 状态检测 | `get_config` / `update_config` / `check_binance_status` | ✅ 已检查 |
...
| 模块 | 功能点 | 状态 |
|------|--------|------|
| data-layer | PostgreSQL、行情仓储、数据质量、Binance 数据源 | ✅ 已检查；Redis 已收敛到 clients |
| clients | Redis 缓存单一实现 | ✅ 已收敛重复实现 |
| repository | 策略/回测仓储、CAS 状态更新 | ✅ 已检查 |
| services | Auth/Account/Market/Strategy/Risk/Binance/Config | ✅ 已检查；AppServices 装配去重 |
| strategy-layer | 指标、信号、回测、注册中心、调度器 | ✅ 已修复调度器 symbols 空实现 |
| trading-layer | OrderManager、执行引擎、算法单、Binance executor | ✅ 已接入 Binance 订单状态查询 |
| risk-layer | 事前/实时/事后/VaR | ✅ 已检查 |
| monitor-layer | 指标、日志、告警 | ✅ 已修复 HTTP 客户端与测试稳定性 |
| security | 加密、JWT、API key、审计 | ✅ 已检查 |
| exchange-binance | REST、WebSocket、模拟数据 | ✅ 已新增单频道退订 |

## 3. 本轮修复清单

- `StrategyScheduler` 从策略参数读取 `symbols`，不再使用空市场数据执行调度任务。
- `BinanceExecutor::get_order_status` 已通过 `get_order_details` 查询并映射订单状态。
- `DataPuller` 重试在 `max_attempts = 0` 时不再 panic。
- `get_market_data` 不再把账户资金伪装成行情数据，数据源不可用时明确报错。
- Binance WebSocket 新增 `unsubscribe_public`，前端“取消订阅”不再停止整个连接。
- Binance WebSocket 支持运行中动态订阅、优雅停止、按配置选择实盘/模拟盘，并修复心跳饿死问题。
- 密码修改后通过 `useAuthStore().clearSession()` 清理统一会话，不再删除错误 key。
- `BinancePlaceOrderRequest` 修正为 Rust `camelCase` 契约，金额/价格使用字符串。
- `Trading.vue` 移除 `any` 并补齐 `AccountInfo`、`Order`、Binance 类型。
