...
pub async fn submit_order(state: State<'_, AppState>, order: Order) -> Result<String, String> {
    ...
    // 记录订单提交日志
    state.log_buffer.add_entry(quant_common::types::LogEntry {
        timestamp: Utc::now(),
...
/// 确认告警
#[tauri::command]
pub async fn acknowledge_alert(state: State<'_, AppState>, alert_id: String) -> Result<bool, String> {
    let uuid = Uuid::parse_str(&alert_id)
...
/// 获取日志信息
#[tauri::command]
pub async fn get_logs(state: State<'_, AppState>, level: Option<String>, _limit: Option<u32>) -> Result<Vec<quant_common::types::LogEntry>, String> {
...
/// 执行事前风控检查
#[tauri::command]
pub async fn pre_trade_check(state: State<'_, AppState>, order: Order, account: Account, positions: Vec<Position>) -> Result<bool, String> {
...
/// 修改密码
#[tauri::command]
pub async fn change_password(_current_password: String, _new_password: String) -> Result<bool, String> {
...
/// 获取 OKX 账户余额
#[tauri::command]
pub async fn get_okx_balance(state: State<'_, AppState>, ccy: Option<String>) -> Result<Vec<OkxBalance>, String> {
    let okx_client_opt = state.okx_client.read().await;
    
    match okx_client_opt.as_ref() {
        Some(client_arc) => {
            let client = client_arc.read().await;
            let balances = client.get_account_balance(ccy.as_deref())
                .await
            ...
            state.log_buffer.add_entry(quant_common::types::LogEntry {
                timestamp: Utc::now(),
                level: "info".to_string(),
                message: format!("Retrieved OKX balance for {}", ccy.unwrap_or_else(|| "all currencies".to_string())),
...
/// 获取 OKX 持仓
#[tauri::command]
pub async fn get_okx_positions(state: State<'_, AppState>, inst_id: Option<String>) -> Result<Vec<OkxPosition>, String> {
    let okx_client_opt = state.okx_client.read().await;
    
    match okx_client_opt.as_ref() {
        Some(client_arc) => {
            let client = client_arc.read().await;
            let positions = client.get_positions(inst_id.as_deref())
                .await
            ...
            state.log_buffer.add_entry(quant_common::types::LogEntry {
                timestamp: Utc::now(),
                level: "info".to_string(),
                message: format!("Retrieved OKX positions for {}", inst_id.unwrap_or_else(|| "all instruments".to_string())),
                module: Some("okx".to_string()),
            }).await;
...
/// 下单到 OKX
#[tauri::command]
pub async fn place_okx_order(state: State<'_, AppState>, request: OkxPlaceOrderRequest) -> Result<OkxOrder, String> {
    let okx_client_opt = state.okx_client.read().await;
    
    match okx_client_opt.as_ref() {
        Some(client_arc) => {
            let client = client_arc.read().await;
            let order = client.place_order(request.clone())
                .await
            ...
            state.log_buffer.add_entry(quant_common::types::LogEntry {
                timestamp: Utc::now(),
                level: "info".to_string(),
                message: format!("Placed OKX order: {} {} {} @ {}", 
                    request.side, request.sz, request.inst_id, 
...
/// 撤销 OKX 订单
#[tauri::command]
pub async fn cancel_okx_order(state: State<'_, AppState>, inst_id: String, ord_id: String) -> Result<bool, String> {
    let okx_client_opt = state.okx_client.read().await;
...
pub async fn get_okx_candles(
    ...
    match okx_client_opt.as_ref() {
        Some(client_arc) => {
            let client = client_arc.read().await;
            let candles = client.get_candles(&inst_id, &bar, limit)
                .await
...
pub async fn get_okx_instruments(
    ...
    match okx_client_opt.as_ref() {
        Some(client_arc) => {
            let client = client_arc.read().await;
            let instruments = client.get_instruments(&inst_type)
                .await
...
/// 获取 OKX 公告
#[tauri::command]
pub async fn get_okx_announcements(state: State<'_, AppState>) -> Result<serde_json::Value, String> {
    let okx_client_opt = state.okx_client.read().await;
    
    match okx_client_opt.as_ref() {
        Some(client_arc) => {
            let client = client_arc.read().await;
            let announcements = client.get_announcements()
                .await
...
/// 使用 OKX 执行器下单
#[tauri::command]
pub async fn execute_okx_order(
    state: State<'_, AppState>,
    order: Order,
            ...
            match result {
                Ok(ord_id) => {
                    ...
                    state.log_buffer.add_entry(quant_common::types::LogEntry {
                        timestamp: Utc::now(),
                        level: "info".to_string(),
                        message: format!("Executed order {} on OKX: {}", order.order_id, ord_id),
