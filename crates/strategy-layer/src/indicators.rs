...
pub fn ema(data: &[Decimal], period: usize) -> Vec<Decimal> {
    ...
    for i in period..data.len() {
        let ema_value = (data[i] - result[result.len() - 1]) * multiplier + result[result.len() - 1];
...
pub fn bollinger_bands(data: &[Decimal], period: usize, std_dev_multiplier: Decimal) -> BollingerBands {
...
pub fn macd(data: &[Decimal], fast_period: usize, slow_period: usize, signal_period: usize) -> MACD {
