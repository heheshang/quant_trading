...
impl MockStrategyRepo {
    fn new() -> Self { Self { state: Arc::new(Mutex::new(HashMap::new())) } }
...
impl StrategyRepository for MockStrategyRepo {
    async fn find_all(&self, _: Option<&str>, _: Option<StrategyType>, _: Option<StrategyStatus>, _: Option<bool>, _: i64, _: i64) -> Result<(Vec<StrategySummaryRow>, i64), RepoError> { stub("not used in CAS test") }
    async fn count(&self, _: Option<&str>, _: Option<StrategyType>, _: Option<StrategyStatus>, _: Option<bool>) -> Result<i64, RepoError> { stub("not used in CAS test") }
    async fn find_by_id(&self, _: &str) -> Result<Option<StrategyParams>, RepoError> { Ok(None) }
    async fn insert(&self, _: &StrategyParams) -> Result<i32, RepoError> { stub("not used in CAS test") }
    ...
    async fn update_status_if(&self, id: &str, new: StrategyStatus, expected: StrategyStatus, _: Option<&str>) -> Result<bool, RepoError> {
        ...
        match s.get(id).copied() {
            Some(cur) if cur == expected => { s.insert(id.to_string(), new); Ok(true) }
            _ => Ok(false),
        }
    }
    async fn stats(&self) -> Result<StrategyStats, RepoError> { stub("not used in CAS test") }
}
...
async fn test_update_status_if_concurrent_only_one_wins() {
    ...
    let (res_a, res_b) = tokio::join!(
        async move { a.update_status_if("strat_cas_001", StrategyStatus::Running, StrategyStatus::Draft, None).await },
        async move { b.update_status_if("strat_cas_001", StrategyStatus::Running, StrategyStatus::Draft, None).await },
    );
    let oks = [&res_a, &res_b].iter().filter(|r| matches!(r, Ok(true))).count();
    let losers = [&res_a, &res_b].iter().filter(|r| matches!(r, Ok(false))).count();
    assert_eq!(oks, 1, "expected exactly one CAS winner, got: {:?} / {:?}", res_a, res_b);
    assert_eq!(losers, 1, "expected exactly one CAS loser, got: {:?} / {:?}", res_a, res_b);
}
