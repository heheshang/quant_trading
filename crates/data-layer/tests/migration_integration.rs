use data_layer::{MigrationManager, Migration};
...
async fn cleanup_test_db(pool: &PgPool) -> Result<()> {
    ...
    let _ = sqlx::query("DROP TABLE IF EXISTS risk_metrics CASCADE").execute(pool).await;
    let _ = sqlx::query("DROP TABLE IF EXISTS alerts CASCADE").execute(pool).await;
    let _ = sqlx::query("DROP TABLE IF EXISTS trades CASCADE").execute(pool).await;
    let _ = sqlx::query("DROP TABLE IF EXISTS positions CASCADE").execute(pool).await;
    let _ = sqlx::query("DROP TABLE IF EXISTS orders CASCADE").execute(pool).await;
    let _ = sqlx::query("DROP TABLE IF EXISTS accounts CASCADE").execute(pool).await;
    let _ = sqlx::query("DROP TABLE IF EXISTS migrations CASCADE").execute(pool).await;
    Ok(())
}
...
async fn test_full_migration_cycle() {
    ...
    manager.init().await.expect("Failed to initialize migrations table");
    ...
    let pending = manager.get_pending_migrations().await.expect("Failed to get pending");
    ...
    let tables = vec!["accounts", "orders", "positions", "trades", "alerts", "risk_metrics"];
    for table in tables {
    ...
    let new_version = manager.get_current_version().await.expect("Failed to get version");
    ...
    let records = manager.get_applied_migrations().await.expect("Failed to get records");
    ...
    assert!(!alerts_exists, "Alerts table should not exist after rollback");
    
    // Test 10: Rollback to version 0
    manager.rollback_to(0).await.expect("Failed to rollback to 0");
    let version = manager.get_current_version().await.expect("Failed to get version");
    ...
    for table in vec!["accounts", "orders", "positions", "trades"] {
        ...
        .await
        .expect(&format!("Failed to check {} table", table));
        assert!(!exists, "Table {} should not exist after full rollback", table);
    }
...
async fn test_migration_idempotency() {
    ...
    manager.migrate().await.expect("First migration should succeed");
    manager.migrate().await.expect("Second migration should succeed (no-op)");
    ...
    let records = manager.get_applied_migrations().await.expect("Failed to get records");
    assert_eq!(records.len(), 3, "Should still have exactly 3 migration records");
...
async fn test_table_structure() {
    ...
    let expected_columns = vec![
        "account_id", "account_name", "account_type", "total_assets",
        "available_balance", "frozen_balance", "margin_ratio", "daily_pnl",
